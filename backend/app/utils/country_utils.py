...
def get_country_name_from_code(country_code: str) -> Optional[str]:
    ...
    try:
        ...
        logger.warning(
            f"Error getting country name for code '{country_code}': {str(e)}"
        )
        return None
...
def get_country_code_from_name(country_name: str) -> Optional[str]:
    ...
    try:
        ...
        logger.warning(
            f"Error getting country code for name '{country_name}': {str(e)}"
        )
        return None
...
def get_all_countries() -> list[dict]:
    ...
    try:
        ...
        for country in pycountry.countries:
            countries.append({"code": country.alpha_2, "name": country.name})
