...
class ChatService:
    ...
    async def chat_stream(
        ...
        if last_message:
            await _fetch_notes(last_message, query_text, user)
            await _fetch_documents(
                last_message, query_text, body.conversation_id, user_id
            )

        # Run this in the background (do not await)
        background_tasks.add_task(_store_note, query_text, user_id)

        if body.pageFetchURL and last_message:
            await _fetch_webpage(last_message, body.pageFetchURL)

        if body.search_web and last_message:
            await _do_search(last_message, query_text)
