...
def create_todo_content_for_embedding(todo_data: dict) -> str:
    ...
    if todo_data.get('subtasks'):
        subtask_titles = [subtask.get('title', '') for subtask in todo_data['subtasks'] if subtask.get('title')]
        if subtask_titles:
...
async def semantic_search_todos(
    ...
    try:
        ...
        if completed is not None:
            filter_conditions.append({"completed": str(completed).lower()})  # Convert to "true" or "false"
    ...
    **filters
    ...
    try:
        ...
        if filters.get('completed') is not None:
            traditional_results = [t for t in traditional_results if t.completed == filters['completed']]
        if filters.get('priority'):
            traditional_results = [t for t in traditional_results if t.priority == filters['priority']]
        if filters.get('project_id'):
            traditional_results = [t for t in traditional_results if t.project_id == filters['project_id']]
        ...
        sorted_todo_ids = sorted(combined_scores.keys(), key=lambda x: combined_scores[x], reverse=True)
