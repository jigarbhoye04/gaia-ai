...
async def _process_subscription_webhook(event_type: str, subscription_entity: Dict[str, Any]) -> Dict[str, str]:
    ...
    if event_type == "subscription.activated":
        ...
        try:
            ...
            complete_subscription = razorpay_service.client.subscription.fetch(subscription_id)
            
            update_data = {
                "status": "active",
                "current_start": timestamp_to_datetime(complete_subscription.get("current_start")),
                "current_end": timestamp_to_datetime(complete_subscription.get("current_end")),
                "charge_at": timestamp_to_datetime(complete_subscription.get("charge_at")),
                "start_at": timestamp_to_datetime(complete_subscription.get("start_at")),
            ...
            await subscriptions_collection.update_one(
                {"razorpay_subscription_id": subscription_id},
                {
                    "$set": {
                        "status": "active",
                        "current_start": timestamp_to_datetime(subscription_entity.get("current_start")),
                        "current_end": timestamp_to_datetime(subscription_entity.get("current_end")),
                        "updated_at": datetime.utcnow(),
                    }
                }
            )
        ...
        try:
            ...
            await subscriptions_collection.update_one(
                ...
                {
                    "$set": update_data,
                    "$inc": {"paid_count": 1} if "paid_count" not in update_data else {}
                }
            )
        except Exception as e:
            logger.error(f"Failed to fetch complete subscription details: {e}")
