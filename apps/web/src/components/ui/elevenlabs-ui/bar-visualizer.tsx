...
export const BarVisualizer = ({
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      } else if (isActiveState && analyserRef.current && audioContextRef.current?.state === 'running') {
      ...
      for (let i = 0; i < barCount; i++) {
        ...
        if (isActiveState && volume > 0.01) {
          ...
          const addedHeight = volume * dist * (maxHeight - minHeight) * (0.8 + noise * 0.2);
        ...
        currentHeights.current[i] = lerp(currentHeights.current[i], targetHeight, 0.18);
  ...
  const getBarColor = () => {
    switch (state) {
      ...
      case "connecting":
      case "initializing": return "bg-primary";
