...
def _process_message_batch(service, message_ids: List[str], filter_query: str | None = None) -> Dict[str, Dict[str, str]]:
    ...
    try:
        ...
        def add_message_callback(request_id, response, exception):
            if exception:
                logger.warning(f"CONTACT_SERVICE: Error in batch request for message {request_id}: {exception}")
                return
                            ...
                            if "@" in email and "." in email:
                                ...
                                if filter_query:
                                    ...
                                    if (query_lower in name_lower or 
                                        query_lower in email_lower or 
                                        name_lower.startswith(query_lower) or
                                        email_lower.startswith(query_lower)):
                                        contact_dict[email] = {"name": name, "email": email}
                                else:
                                    contact_dict[email] = {"name": name, "email": email}
                                    
            except Exception as e:
                logger.warning(f"CONTACT_SERVICE: Error processing message in batch: {e}")
...
def _process_messages_individually(service, message_ids: List[str], filter_query: str | None = None) -> Dict[str, Dict[str, str]]:
    """
                    ...
                    for address in addresses:
                        ...
                        if "@" in email and "." in email:
                            if filter_query:
                                ...
                                if (query_lower in name_lower or query_lower in email_lower or 
                                    name_lower.startswith(query_lower) or email_lower.startswith(query_lower)):
                                    contact_dict[email] = {"name": name, "email": email}
                            else:
                                contact_dict[email] = {"name": name, "email": email}
...
def extract_contacts_from_messages_batch(service, message_ids: List[str], filter_query: str | None = None, batch_size: int = 50) -> List[Dict[str, str]]:
    """
    ...
    """
    logger.info(f"CONTACT_SERVICE: Starting to extract contacts from {len(message_ids)} messages using batch processing")
    ...
    for batch_start in range(0, len(message_ids), batch_size):
        ...
        logger.debug(f"CONTACT_SERVICE: Processing batch {batch_start//batch_size + 1}: {len(batch_ids)} messages")
    ...
    logger.info(f"CONTACT_SERVICE: Extracted {len(contacts)} unique contacts from {len(message_ids)} messages")
...
def get_gmail_contacts(
    ...
    """
    try:
        logger.info(f"CONTACT_SERVICE: Starting contact search with query: '{query}', max_results: {max_results}")
        ...
        if message_ids:
            logger.info(f"CONTACT_SERVICE: Extracting contacts from {len(message_ids)} message IDs using batch processing")
            contacts = extract_contacts_from_messages_batch(service, message_ids, filter_query=query)
            ...
            logger.info(f"CONTACT_SERVICE: Returning {len(contacts)} contacts for query '{query}'")
            return result
        else:
