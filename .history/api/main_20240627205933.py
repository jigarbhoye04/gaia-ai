...
@app.post("/chat", response_model=List[MessageResponse])
async def doPrompt(request: MessageRequest):
    user_message: str = request.message

    print(user_message)

    # Assuming client.chat.completions.create() returns an async generator
    async_generator = client.chat.completions.create(
        model="llama3-8b",
        messages=[
            {"role": "system",
    ...
    async for response in async_generator:
        ...
        responses.append(MessageResponse(response=chatbot_response))
