...
class GoogleDocUpdate(BaseModel):
    ...
    insert_at_end: bool = Field(True, description="Whether to insert at end or replace all content")
...
class GoogleDocFormat(BaseModel):
    ...
    foreground_color: Optional[Dict[str, float]] = Field(None, description="Text color as RGB values (0-1)")
...
class GoogleDocList(BaseModel):
    ...
    query: Optional[str] = Field(None, description="Optional search query to filter documents")
...
class GoogleDocResponse(BaseModel):
    ...
    modified_time: Optional[str] = Field(None, description="Last modification timestamp")
...
class GoogleDocFormatResponse(BaseModel):
    ...
    document_id: str = Field(..., description="ID of the document")
    url: str = Field(..., description="URL to access the document")
    formatting_applied: int = Field(..., description="Number of formatting operations applied")
