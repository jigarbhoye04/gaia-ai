...
async def bulk_move_todos(todo_ids: List[str], project_id: str, user_id: str) -> List[TodoResponse]:
    ...
    try:
        ...
        old_project_ids = set(todo.get("project_id") for todo in old_todos if todo.get("project_id"))
        ...
        todos_logger.info(f"Bulk moved {result.modified_count} todos to project {project_id} for user {user_id}")
...
async def bulk_delete_todos(todo_ids: List[str], user_id: str) -> None:
    ...
    try:
        ...
        project_ids = set(todo.get("project_id") for todo in todos_to_delete if todo.get("project_id"))
        ...
        todos_logger.info(f"Bulk deleted {result.deleted_count} todos for user {user_id}")
