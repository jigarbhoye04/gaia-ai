...
# Load backend .env (from parent backend directory)
...
# Inject Infisical secrets (using local voice_secrets.py)
try:
    from src.voice_secrets import inject_infisical_secrets
    inject_infisical_secrets()
except ImportError as e:
    print(f"⚠️ inject_infisical_secrets() not found or failed to import: {e}")
except Exception as e:
    print(f"⚠️ Failed to load Infisical secrets: {e}")

# Define VoiceSettings model
