...
export function useChat(chatId: string | null): UseChatReturn {
  ...
  const messages = useChatStore(
    useShallow((state) => 
      currentConversationId 
        ? state.messagesByConversation[currentConversationId] ?? EMPTY_MESSAGES 
        : EMPTY_MESSAGES
    )
  );
  
  const streamingState = useChatStore(useShallow((state) => state.streamingState));
  const isTyping = streamingState.isTyping && streamingState.conversationId === currentConversationId;
  const progress = streamingState.conversationId === currentConversationId ? streamingState.progress : null;
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    store.setStreamingState({ isTyping: true, isStreaming: true, conversationId: storeKey });
      ...
      const controller = await fetchChatStream(
        ...
        },
        {
          ...
          onFollowUpActions: (actions) => {
            useChatStore.getState().updateLastMessageFollowUp(activeConvIdRef.current!, actions);
          },
          onDone: () => {
            useChatStore.getState().setStreamingState({ 
              isTyping: false, 
